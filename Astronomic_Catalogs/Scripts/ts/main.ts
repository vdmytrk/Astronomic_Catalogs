...
// Це просто типи, які можна використовувати для кращої типізації. У твоєму випадку вони необов’язкові, 
//   бо ти їх ніде не використовуєш напряму. Можеш їх видалити, і код працюватиме так само.
// Тобто, якщо ти хочеш мінімальні зміни, просто видали ці рядки, і все буде добре!
type AjaxFailHandler = (jqXHR: AjaxResponse) => void;
type AjaxDoneHandler = (data: any) => void;
type AjaxAlwaysHandler = () => void;
// Ці рякди вказують Webpack, що глобальні функції на яких немає посилання в TS-коді не видаляти і 
//   і не перейменовувати, адже вони можуть викликатися стороннім кодом. В даному випадку кнопками з відображень.
